            ...
            if (source instanceof Player) {
                ...
                if (plugin.getPvpClassManager().hasClassEquipped(shooter, this)) {
                    ...
                    if (newLevel >= MARK_EXECUTION_LEVEL) {
                        ...
                        attacked.sendMessage(ChatColor.GOLD + "Marked! " + ConfigurationService.ENEMY_COLOUR + shooter.getName() + ChatColor.GOLD + " hit you with a level " + ChatColor.WHITE
                                + ChatColor.GOLD + newLevel + " mark.");
                        shooter.sendMessage(ChatColor.YELLOW + "Marked! Hit " + ConfigurationService.ENEMY_COLOUR + attacked.getName() + ChatColor.YELLOW + " with a level " + ChatColor.WHITE
                        ...
                        archerMark.decrementTask = new BukkitRunnable() {
                            ...
                            public void run() {
                                ...
                                if (newLevel == 0) {
                                    ...
                                    shooter.sendMessage(ChatColor.GOLD + "You no longer have a mark on " + ConfigurationService.ENEMY_COLOUR + attacked.getName() + ChatColor.GOLD + '.');
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (action == Action.RIGHT_CLICK_AIR || action == Action.RIGHT_CLICK_BLOCK) {
            if (event.hasItem() && event.getItem().getType() == Material.SUGAR) {
                ...
                if (remaining > 0L) {
                    player.sendMessage(ChatColor.RED + "Still on " + getName() + " speed cooldown for another " + DurationFormatUtils.formatDurationWords(remaining, true, true) + ".");
                } else {
